...
uploaded_file = st.file_uploader("Upload an image of any fashion product...", type=["jpg", "jpeg", "png"])
if uploaded_file is not None:
    ...
    if st.button('HOG Image', key='hog_button'):
        hog_image, _ = extract_hog_and_edge(image)
        st.image(hog_image, caption='HOG Image', use_column_width=True)

    if st.button('Edge Detection Image', key='edge_button'):
        _, edge_image = extract_hog_and_edge(image)
        st.image(edge_image, caption='Edge Detection Image', use_column_width=True)
